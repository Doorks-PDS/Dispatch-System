...
def create_customer(request: Request, payload: CustomerCreate, x_api_key: Optional[str] = Header(default=None)):
    _require(request, x_api_key)
    try:
        item = _store(request).create_customer(payload.dict())
...
def update_customer(request: Request, item_id: str, payload: CustomerCreate, x_api_key: Optional[str] = Header(default=None)):
    _require(request, x_api_key)
    try:
...
def list_contacts(
    ...
    _require(request, x_api_key)
    return {"ok": True, "items": _store(request).list_contacts(company_name=company_name, customer_id=customer_id)}
...
def update_contact(request: Request, item_id: str, payload: ContactCreate, x_api_key: Optional[str] = Header(default=None)):
    _require(request, x_api_key)
    try:
