...
def _default_notes_csv_candidates() -> List[Path]:
    # You can point ATLAS_TECH_NOTES_CSV to your exported notes file.
    env = os.getenv("ATLAS_TECH_NOTES_CSV")
    cands: List[Path] = []
    if env:
        cands.append(Path(env).expanduser().resolve())

    # common local paths under project
    here = Path(__file__).resolve()
    proj = here.parents[2]  # doorks/
    cands += [
        proj / "data" / "tech_notes.csv",
        proj / "data" / "tech_notes_export.csv",
    ]
    return cands
