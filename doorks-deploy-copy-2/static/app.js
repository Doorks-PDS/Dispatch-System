(() => {
  ...
  async function apiGenerateDocumentDescription(payload) {
    const data = await fetchJSON(`/documents/auto-description`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
...
function renderEmployeesView() {
  ...
  function defaultDocItems() {
    return [
      ...
      createDocLineItem({ code: "FUEL", description: "Fuel Surcharge", qty: 1, rate: 15, kind: "fee", taxable: false }),
    ];
  }
    ...
    openDrawer("Estimate / Invoice", async (drawerBody, overlay) => {
      ...
      card.innerHTML = `
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:10px; align-items:center;">
          <button class="btn btn-orange" id="doc_auto_fill">Auto Fill Description</button>
          <button class="btn" id="doc_auto_prompt">Prompt Builder</button>
          <div class="hint">Uses your PDS wording style from saved job details, tech notes, and recommendations.</div>
        </div>
        <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:10px; align-items:center;">
      ...
      function mergeSuggestedItems(suggested){
        const next = Array.isArray(items) ? items.slice() : [];
        (Array.isArray(suggested) ? suggested : []).forEach(s => {
          const code = String(s.code || '').trim().toLowerCase();
          const desc = String(s.description || '').trim().toLowerCase();
          const match = next.find(it => String(it.code || '').trim().toLowerCase() === code && String(it.description || '').trim().toLowerCase() === desc);
          if (!match) next.push(createDocLineItem(s));
        });
        items = next;
      }
      async function autoFillDocument(usePrompt) {
        try {
          const extraNotes = usePrompt ? (prompt('Enter any extra notes or site-specific scope details for this document:', '') || '') : '';
          const scopeHint = usePrompt ? (prompt('Enter any specific scope wording you want included (optional):', '') || '') : '';
          const suggestion = await apiGenerateDocumentDescription({ job_id: job ? job.id : ((editDoc && editDoc.job_id) || ''), doc_type: docType, extra_notes: extraNotes, scope_hint: scopeHint });
          if (suggestion && typeof suggestion.work === 'string' && suggestion.work.trim()) dom.work.value = suggestion.work;
          if (suggestion && Array.isArray(suggestion.items) && suggestion.items.length) {
            mergeSuggestedItems(suggestion.items);
            renderItems();
          }
        } catch (e) {
          alert(e.message || String(e));
      ...
      card.querySelector("#add_fuel").addEventListener("click", ()=>{ items.push(createDocLineItem({ code:"FUEL", description:"Fuel Surcharge", qty:1, rate:15, kind:"fuel", taxable:false })); renderItems(); });
      ...
      card.querySelector("#doc_auto_fill").addEventListener("click", ()=> autoFillDocument(false));
      card.querySelector("#doc_auto_prompt").addEventListener("click", ()=> autoFillDocument(true));
