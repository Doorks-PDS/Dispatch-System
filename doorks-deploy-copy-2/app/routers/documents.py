...
from app.services.doc_templates import build_doc_suggestion
...
def auto_description(request: Request, payload: AutoDescriptionRequest, x_api_key: Optional[str] = Header(default=None)):
    _require(request, x_api_key)
    job_store = getattr(request.app.state, "calendar_store", None)
    job = {}
    if payload.job_id and job_store:
        found = job_store.get_job(payload.job_id)
        if found:
            job = dict(found)
    suggestion = build_doc_suggestion(job, payload.doc_type, extra_notes=payload.extra_notes, scope_hint=payload.scope_hint)
