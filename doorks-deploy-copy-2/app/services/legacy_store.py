...
class LegacyStore:
    ...
    def load(self) -> None:
        ...
        rows.extend(self._load_csv(first_existing_data_path(self.project_root, "Billable_Time.csv"), "BILLABLE_UPGRADED"))
        rows.extend(self._load_csv(first_existing_data_path(self.project_root, "billable_time.csv"), "BILLABLE"))
        rows.extend(self._load_csv(first_existing_data_path(self.project_root, "Jobs.csv"), "JOBS"))
        rows.extend(self._load_csv(first_existing_data_path(self.project_root, "jobs.csv"), "JOBS"))
        rows.extend(self._load_csv(first_existing_data_path(self.project_root, "tech_notes.csv"), "TECH_NOTES"))
    ...
    def _load_legacy_json(self) -> List[Dict[str, Any]]:
        path = first_existing_data_path(self.project_root, "legacy_jobs.json")
        if not path.exists():
            return []
        try:
